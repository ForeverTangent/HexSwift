...
protocol HConversion {
	static func getPointFromHex(_ hex: Hex, usingLayout layout: HLayout) -> CPoint
	static func getFractionalHexFromCPoint(_ cPoint: CPoint, usingLayout layout: HLayout) -> HFractional
	
}

extension HConversion {
	...
	*/
	...
	static func getFractionalHexFromCPoint(_ cPoint: CPoint, usingLayout layout: HLayout) -> HFractional {
		...
		let orientation = layout.orientation
		let cPoint = CPoint(x: (cPoint.x - layout.origin.x) / layout.size.x,
		...
		return HFractional(q: q, r: r, s: -q - r)
